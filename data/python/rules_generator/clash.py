...
def generate_clash_rules(domains: List[str], output_file: Path) -> None:
    """生成 Clash 格式规则（YAML）"""
    output_file.parent.mkdir(parents=True, exist_ok=True)
    
    with output_file.open('w', encoding='utf-8') as f:
        f.write("# EasyAds Clash 去广告规则\n")
        f.write(f"# 生成时间: {time.strftime('%Y-%m-%d %H:%M:%S')}（北京时间）\n")
        f.write("# 来源: EasyAds 规则集\n")
        f.write("rules:\n")
        
        for domain in sorted(domains):
            # 优先匹配子域（DOMAIN-SUFFIX）
...
if __name__ == "__main__":
    ...
    output_file = root_dir / "clash_rules.yaml"
    
    try:
        domains = extract_domains_from_dns(input_file)
        generate_clash_rules(domains, output_file)
        print(f"成功生成 Clash 规则: {output_file}，共 {len(domains)} 条域名")
